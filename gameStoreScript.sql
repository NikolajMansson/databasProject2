...
CREATE TABLE IF NOT EXISTS `GameShop`.`Item` (
  ...
  PRIMARY KEY (`Platform_Abbreviation`, `Game_Title`),
...
CREATE TABLE IF NOT EXISTS `GameShop`.`RegularCustomerOrder` (
  ...
  `Item_ArticleNumber` VARCHAR(45) NOT NULL,
  `Item_Platform_Abbreviation` VARCHAR(20) NOT NULL,
  `Item_Game_Title` VARCHAR(50) NOT NULL,
  PRIMARY KEY (`OrderNumber`),
  INDEX `fk_Order_Customer1_idx` (`Customer_UserName` ASC),
  INDEX `fk_Order_Employees1_idx` (`Employees_UserName` ASC),
  INDEX `fk_RegularCustomerOrder_Item1_idx` (`Item_Platform_Abbreviation` ASC, `Item_Game_Title` ASC),
  ...
  CONSTRAINT `fk_RegularCustomerOrder_Item1`
    FOREIGN KEY (`Item_Platform_Abbreviation` , `Item_Game_Title`)
    REFERENCES `GameShop`.`Item` (`Platform_Abbreviation` , `Game_Title`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `GameShop`.`GuestOrder` (
  ...
  `Employees_UserName` VARCHAR(45) NOT NULL,
  `DateOfOrder` DATE NOT NULL,
  `Item_InputID` INT NOT NULL,
  `Item_Platform_Abbreviation` VARCHAR(20) NOT NULL,
  `Item_Game_Title` VARCHAR(50) NOT NULL,
  INDEX `fk_GuestOrder_Employees1_idx` (`Employees_UserName` ASC),
  PRIMARY KEY (`OrderNumber`, `Item_InputID`),
  INDEX `fk_GuestOrder_Item1_idx` (`Item_Platform_Abbreviation` ASC, `Item_Game_Title` ASC),
