...
public class BossWelcomeMenuController implements Initializable {
    ...
    private void searchEmployeeContactInfo(ActionEvent ae){
        ...
        FXMLLoader loader = new FXMLLoader ( getClass ().getResource ( "sceneContactEmployee.fxml" ) );
        Parent root = null;
        try {
            root = loader.load ();
        } catch (IOException e) {
    ...
    @FXML
    private void viewLog(ActionEvent ae){
        Node node = (Node) ae.getSource ();
        Stage stage = (Stage) node.getScene ().getWindow ();

        FXMLLoader loader = new FXMLLoader ( getClass ().getResource ( "sceneLog.fxml" ) );
        Parent root = null;
        try {
            root = loader.load ();
        } catch (IOException e) {
